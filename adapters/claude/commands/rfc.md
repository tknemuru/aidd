...
4. **E2E テスト仕様（§7）の起草前に、以下の4段階の導出を内部推論として行う。RFC に書き込むのは Step 6-E2E-4 の最終テーブルのみとし、Step 6-E2E-1〜3 の中間結果は RFC に含めない:**
   - **Step 6-E2E-1**: §2（機能要件）から対象機能を列挙する（内部推論）
   - **Step 6-E2E-2**: コードベースとドメイン知識から各機能の条件軸を列挙する（内部推論）
   - **Step 6-E2E-3**: 実運用の具体的テストデータを導出する（内部推論）
     - 汎用ダミーデータ禁止
     - 「データ取得方法」列を設け、実操作のみ記載可（実 API から取得、実 DB から取得等）
     - モック関連の取得方法は記載不可
   - **Step 6-E2E-4**: Step 6-E2E-1〜3 を統合して §7 テーブルを生成する（これのみ RFC に書き込む）
     - 各機能に対し正常系 + 異常系/境界値の2ケース以上
5. 起草した内容を `docs/rfcs/<slug>/rfc.md` に書き込む。
6. 「ドキュメント編集仕様」セクション起草時に、システム概要ドキュメント（`docs/architecture.md`, `docs/domain-model.md`, `docs/api-overview.md`）への影響を判断し、影響がある場合は更新を対応項目として明記せよ。
...
- [ ] §7 に E2E テスト導出の中間ステップ（対象機能・条件軸・テストデータ）が含まれていないこと
