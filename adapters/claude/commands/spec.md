...
```
/spec <プロダクト情報> [--risk high|medium|low]
```
...
#### Step 1-3: 初期化スクリプトの実行

1. プロダクト情報から slugstr を生成する
...
スクリプトが以下を自動実行する:
- JST日付付き slug（`YYYYMMDD-slugstr`）の生成
- 重複チェック（同名ディレクトリが存在する場合 `-2`, `-3` サフィックスを付与）
- デフォルトブランチの最新化
- `docs/spec-<slug>` ブランチの作成・チェックアウト
- `docs/specs/<slug>/` ディレクトリの作成とテンプレートコピー

スクリプトの標準出力が生成された slug（日付付き）。以降のステップではこの slug を使用する。

#### Step 1-4: テンプレートの読み込み

テンプレート（`docs/specs/<slug>/spec.md`）を Read ツールで読み込む
...
まず §0.5 リスクレベルに Step 1-1a で決定したリスクレベル値を書き込む。
...
起草した内容を `docs/specs/<slug>/spec.md` に書き込む。
...
生成した §2 テーブルで `docs/specs/<slug>/spec.md` を更新する。
...
起草した内容で `docs/specs/<slug>/spec.md` を更新する。
